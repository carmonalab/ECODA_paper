...
1_submit_hpc_array.sh (login; per method: manifest + sbatch array;
  ...
  --partition <P> override drops the constraint pin)
   ...
   └─ NAS reachability check -> OOM-aware sacct gate (fail-closed; an
      OUT_OF_MEMORY task's dataset is re-submitted with doubled --mem, up to
      BENCHMARK_MEM_MAX) ->
...
1_submit_hpc_array.sh (login; per method: manifest + sbatch array on the
  ...
  drops the constraint pin; mofa/pseudobulk auto-prepend prepare_pseudobulk)
   ...
   │    gate (soft; OOM-AWARE fail-closed gate applies under --force or when
   │    variant files are missing — a stale-node prep failure with the
   │    variants already on disk must not block the method arrays, while an
   │    OOM'd prep task auto-escalates its memory)
   ...
   └─ gloscope/mofa/pseudobulk/scitd arrays (after the gate) — poll + OOM-aware
        gate (OUT_OF_MEMORY tasks re-submit only their datasets with doubled
        --mem, up to BENCHMARK_MEM_MAX; non-OOM failures fail closed)
