...
# ============================================================
# Artifact persistence is a shared deep module, not part of the benchmark
# method orchestration implementation. Resolve it from the bound project
# first, then from this file's source location for direct utility callers.
.ecoda_artifact_module_root <- Sys.getenv("PROJECT_ROOT", unset = "")
.ecoda_artifact_module_path <- if (nzchar(.ecoda_artifact_module_root)) {
  file.path(.ecoda_artifact_module_root, "src", "utils", "artifact_records.R")
} else {
  file.path("src", "utils", "artifact_records.R")
}
if (!file.exists(.ecoda_artifact_module_path)) {
  .ecoda_artifact_source_file <- tryCatch(
    sys.frame(1L)$ofile,
    error = function(error) ""
  )
  if (is.character(.ecoda_artifact_source_file) &&
      length(.ecoda_artifact_source_file) == 1L &&
      nzchar(.ecoda_artifact_source_file)) {
    .ecoda_artifact_module_path <- normalizePath(
      file.path(
        dirname(.ecoda_artifact_source_file),
        "..", "utils", "artifact_records.R"
      ),
      mustWork = FALSE
    )
  }
...
if (!file.exists(.ecoda_artifact_module_path)) {
  stop("R artifact records module not found: ", .ecoda_artifact_module_path)
}
source(.ecoda_artifact_module_path)
rm(.ecoda_artifact_module_root, .ecoda_artifact_module_path)
if (exists(".ecoda_artifact_source_file", inherits = FALSE)) {
  rm(.ecoda_artifact_source_file)
}
