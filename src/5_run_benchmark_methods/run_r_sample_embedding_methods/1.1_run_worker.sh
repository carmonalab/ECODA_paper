...
mkdir -p "${OUT_DIR}"
# Log name carries the array JOB id: the per-method arrays run concurrently
# and all use task ids 1..N — without the job id they would read-modify-write
# the same file. The merge script globs per job id.
