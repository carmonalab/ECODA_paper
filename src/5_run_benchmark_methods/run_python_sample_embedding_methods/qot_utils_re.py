"""Vendored copy of qot_utils_re.py from PennShenLab/QOT.

Provenance:
- Repo: https://github.com/PennShenLab/QOT
- Commit: 28cd529880c1 (2024-12-04)
- License: MIT (original header retained below in spirit; the upstream repo
  is distributed under the MIT license)

Modifications (TWO functional changes, plus this header; full traceability in
docs/qot_hotfixes.md):
- Gaussian_Mixture_Representation: the `elif num_samples == 2:` tail was
  changed to an `else` branch so that (sample, cell-type) groups with
  3 <= num_samples < num_components + min_samples_gmm are no longer silently
  dropped from `params` (they previously fell through the if/elif chain,
  leaving 0-distance rows/cols in the QOT_Distance matrix). The new branch
  uses the identical single-Gaussian representation as the == 2 branch.
  The `== 1` and `== 2` branches are untouched.
- Gaussian_Mixture_Representation: added `and num_samples > data.shape[1]` to
  the GMM branch condition. A full-covariance GaussianMixture requires a
  positive-definite empirical covariance, i.e. more samples than PCA dims
  (rank of the covariance of n points in d dims is <= n-1). Without the
  guard, groups with n <= d raise
  "Fitting the mixture model failed ... ill-defined empirical covariance"
  (observed on the _debug dataset: 13 of 35 groups failed, down to 1-cell
  groups which sklearn rejects outright with ensure_min_samples=2; full
  datasets have rare cell types in single cells too). Groups with n <= d now
  fall through to the single-Gaussian branches (== 1, == 2, else). No other
  changes.

Known latent issues left unfixed (dead paths for the ECODA benchmark):
...
- compute_shapley_values / trajectory_analysis are not used by our benchmark.

Import mechanism: this module sits next to 1.1.1_benchmark_methods_py.py and
is importable only because that script's directory is sys.path[0] when run as
`python <path>/1.1.1_benchmark_methods_py.py` (not `-m`). Keep the flat
layout — do NOT convert to a package-relative import.
"""
...
def Gaussian_Mixture_Representation(adata, num_components=5, random_state=2, min_samples_gmm=0):
    ...
    for (source, label), group in grouped:
        ...
        else:  # 2 <= num_samples < num_components + min_samples_gmm, use mean
            # MODIFIED (was `elif num_samples == 2:`): groups with
            # 3 <= num_samples < num_components + min_samples_gmm fell
            # through the if/elif chain and were silently dropped from
            # params, leaving 0-distance rows/cols in QOT_Distance.
            # Single-Gaussian representation identical to the == 2 branch.
