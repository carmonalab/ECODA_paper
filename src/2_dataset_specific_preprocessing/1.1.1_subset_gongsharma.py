...
- ORDERING (race hazard): `1_submit_hpc.sh` submits this step FIRST and gates
  the generic worker (`1.submit.sh --step combinedpbmc`) behind it via
  `--dependency=afterok`, because that worker reads the SAME staged files in
  backed mode — an in-place overwrite racing that read would nondeterminize
  the CombinedPBMC dataset.

Usage (HPC, via `1.submit.sh --step gongsharma_cap`; pure Python, no R interop):
