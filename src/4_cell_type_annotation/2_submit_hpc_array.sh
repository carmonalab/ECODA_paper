...
if ls "${NAS_TARGET_DIR}/.." > /dev/null 2>&1; then
    ...
    for DS_DIR in "${SYNC_DIRS[@]}"; do
      ...
      rsync -rlptDv --exclude='annotations_chunk_*.feather' --exclude='annotation_stats_chunk_*.feather' --exclude='chunks/' --exclude='annotation_tmp/' "${DS_DIR}/" "${NAS_TARGET_DIR}/${DS_NAME}/output/"
