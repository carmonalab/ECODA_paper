...
_ecoda_expand_output_selection() {
  local stage="${1:-}" selection="${2:-}" step script ds view outputs extra
  local path name old_ifs expected_script expected_outputs write_flag
  _ecoda_init_output_arrays
  ...
  case "${stage}" in
    ...
    stage3|stage4)
      ...
      ;;
    *)
      _ecoda_die "unsupported stage for shared ownership expansion: ${stage}"
      return 1
      ;;
  esac
...
_ecoda_validate_expanded_output_ownership() {
  local stage="${1:-}" run_id="${2:-}" reclaim_terminal="${3:-0}"
  ...
  ecoda_validate_run_id "${run_id}" || return 1
  [[ "${stage}" =~ ^[A-Za-z][A-Za-z0-9_-]*$ ]] || {
    _ecoda_die "invalid output ownership stage: ${stage}"
    return 1
  }
  _ecoda_validate_output_owner_availability "${run_id}" || return 1
  [[ ${#ECODA_OUTPUT_PATHS[@]} -gt 0 ]] || {
    _ecoda_die "selection expands to no output artifacts"
    return 1
  }
  for path in "${ECODA_OUTPUT_PATHS[@]}"; do
...
ecoda_validate_output_ownership() {
  ...
  [[ "${stage}" == stage2 || "${stage}" == stage3 ||
     "${stage}" == stage4 ]] || {
    _ecoda_die "shared output ownership does not implement stage policy: ${stage}"
    return 1
  }
  [[ "${selection}" = /* ]] || {
...
# dataset identity, view identity, and an optional third label field.
