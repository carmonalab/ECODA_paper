...
- **HPC wrappers** (`src/`): SLURM scripts for parallel per-dataset execution. `slurm_bash_config.env`, `preprocess/1_submit_hpc_array.sh` (stages data + submits array), `cell_type_annotation/` (array job pipeline). Planned: `submit_benchmark_methods_py.sh`.
